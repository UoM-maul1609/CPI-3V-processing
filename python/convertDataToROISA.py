import numpy as np
import struct
def convertDataToROISA(bytes,ushort,order,rois):
    ...
    for i in range(len(rois)):
        ...
        R['ulItemSize'][i],=struct.unpack('I', bytes[ro+2:ro+2+4]) 
        ...
        R['PixBytes'][i],=struct.unpack('h', bytes[ro+16:ro+16+2]) 
        R['usROIFlags'][i]=ushort[rois[i]+9]
        
        R['fLength'][i],=struct.unpack('f', bytes[ro+20:ro+20+4]) 

        R['ulStartLen'][i],=struct.unpack('I', bytes[ro+24:ro+24+4]) 
        R['ulEndLen'][i],=struct.unpack('I', bytes[ro+28:ro+28+4]) 
        
        R['fWidth'][i],=struct.unpack('f', bytes[ro+32:ro+32+4]) 

        R['Spare'][i]=np.reshape(bytearray(bytes[ro+36:ro+36+18]),(18,1))
